...
void MMU_init(){
    ...
    L1_block_descriptor_NORMAL_init(0);
...
void L2_block_descriptor_DEVICE_init(int _index){
    ...
    L2_table[_index] &= ~(3ULL << 2); //Указание индекса в MAIR(т.е. указывается тип памяти: Device | Normal). В данном случаи: индекс 0, см. в MMU_init()
    L2_table[_index] &= ~(1ULL << 7); //Указание флага доступа. В данном случаи RW Priv
    L2_table[_index] &= ~(3ULL << 8); //Указание того, что между ядрами данный дескриптор не делится на чтение
